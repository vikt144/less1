...
public int Output_=1,Input_=1; // input output  = 0 terminal  б 1 buffer   ///////////////input output\\\\\\\\\\\\

public StringBuilder  StringBuffer_chan = null;

public void output_chan(String str ) {
  if ( Output_==0 ) System.out.println(str);
    else 
       if (StringBuffer_chan==null) 
          StringBuffer_chan = new StringBuilder();
       StringBuffer_chan.append(str);
}

public String input_chan()  {
   String ret = null;
   if (StringBuffer_chan!=null)    
            ret = new String (StringBuffer_chan); 
   StringBuffer_chan=null; 
   return ret;
}

public void read_chan() {
    ST.pop(); //сбросить стек
    String s = input_chan();
...
public  void ascii() {  ST.push(32); WORD();
 ...
 public void qstring() {  // s"  на верш длина 
  ...
  public void printIt(String s) {
      ...
      output_chan(utf_ ); // конвертация в utf8
      } catch (Exception e ) {} 
  }
...
public void FIND() {
  ...
  public void  gui_setText() {  // int o, int i , int op ,String, s s.length)  obj  num  op  s" ghjh "   s sl bn 
   Message msg;
   int l =  ST.pop();
   int indStr =  ST.pop(); 
   int op  =  ST.pop();  // op  0 get , 1 set
   int ind =  ST.pop();   // text0
   int obj =  ST.pop();  // 1 button 0 textView
   String s = getStringv(StringVector, indStr);
//   msg = Hnd.obtainMessage(0, 0, 0, s);
   msg = Hnd.obtainMessage(obj, ind, op, s);
            // отправляем
            Hnd.sendMessage(msg);   
  }

public void smpMess(int t , int arg , String txt) {  //0 text0  . arg any num , text 
 //   int t =  ST.pop();  // 0 text0
    Message msg;
    msg = HndMes.obtainMessage(t, arg , 0 , txt);
            // отправляем
    HndMes.sendMessage(msg);
}
  ...
  if (port==8)  { /* System.out.println("port=8 ") */ ;runProcedure( Ob , procedureLists[y]);
   ...
   if (port != 0) System.out.println("нетот порт "+ port);
     ...
     switch (y) {
       ...
       case 1 : printIt(stack.pop()+"");// System.out.print( /*"st= "+ */stack.pop()+" " );
          break;
...
//                                                            reflections api
 public Method [] procedureLists = new Method[150];
 public int procedureIndex=0;
...
//  Class<fas> carClass = fas.class;
  Class ob = o .getClass();  
//  Class<ob> carClass = ob.class;  
  Field[] declaredFields = ob.getDeclaredFields();  //  carClass.getDeclaredFields();
...
"   word WORD n   find FIND n  print print n   literal literal n" +
 ...
 "     (  comment  i    "   + 
 " ascii ascii n    s\" qstring i    type stype n    s+ sconcat n    spick spick n" +
 " s= sequals n " +
...
//float words
 ...
 " gui_setText gui_setText n    readchan  read_chan n " +
...
// fln flog  fsqrt fexp fabs floor  d>f f>d
  ...
  public void fprint_()  {
    printIt(ST.fpop()+"");  // System.out.println("dbl=" + ST.fpop() );
  }
  public void fstore() {
     int adr = ST.pop();
...
public void comma() {
 ...
 public void interpret() {
  boolean log = true;  
  error = 0; ErrorString = null;
//  smpMess(1 , 0 ," interpretStart" );   
  while ( log ) { 
    String wrl; 
    ST.push(32); // разделитель для word  
    WORD();    // на стеке 0
...
//    System.out.println("n= " +  n );
		...
		//	   System.out.println("проверка число ");
		           if (error != 0)  {   // если число - number оставляет 0
			                   log = false;
		 	  		   ErrorString = "слово " +  wrl + " не найдено"; //System.out.println("слово " +  wrl + " не найдено" );
					   error = 0;
					   smpMess(1 , 0 , ErrorString  );
					   } else {  // literal() ; 
 ...
 public  String loadTextFile(String filename) {
   ...
   try { 
	    ...
	    fis.close();
            ...
            smpMess(1 , 0 , "er " + e  ); //System.out.println(e.getMessage());
        }
 return ret;
 }
